...
Original file is located at
    https://colab.research.google.com/drive/1dURJl_wKMhLifUSva4oUgYjjejuYq2iv
...
#     shutil.rmtree(DATASET_DIR) # ファイル削除
#
# categories = []
...
def training(model, dataset, optimizer):
    model.train()
...
model = SimpleAttentionNetwork(len(train_dataset.classes)).to("cuda")
...
for epoch in range(100):
    train_loss, train_acc = training(model, train_dataset, optimizer)
...
# Commented out IPython magic to ensure Python compatibility.
# %%bash
# FILE_ID=1XmclSYkbPXFUoDI6XAWj3Nvvs61Kz6rN
# FILE_NAME=best_simple.pth
# curl -sc /tmp/cookie "https://drive.google.com/uc?export=download&id=${FILE_ID}" > /dev/null
# CODE="$(awk '/_warning_/ {print $NF}' /tmp/cookie)"  
# curl -Lb /tmp/cookie "https://drive.google.com/uc?export=download&confirm=${CODE}&id=${FILE_ID}" -o ${FILE_NAME}

model = SimpleAttentionNetwork(len(train_dataset.classes)).to("cuda")
model.load_state_dict(torch.load(open("best_simple.pth", 'rb')))
